      ...
      <main className="flex-grow">
        ...
        {activeTab === "home" && (
          <div className="bg-slate-50">
            ...
            {/* TMX PARTNERSHIP & CLIENT LOGOS SECTION */}
            <section className="py-16 px-6 max-w-7xl mx-auto space-y-16 border-t border-slate-200">
              {/* Clients Logos */}
              <div className="space-y-8">
                <div className="text-center max-w-2xl mx-auto space-y-2">
                  <span className="text-[#155E54] font-mono text-[10px] tracking-widest uppercase font-semibold">Portfólio de Atendimento</span>
                  <h3 className="text-xl font-bold text-slate-900 uppercase tracking-wide">Empresas que Confiam em Nossos Serviços</h3>
                  <p className="text-slate-500 text-xs">Algumas das principais marcas, laticínios, cooperativas e indústrias que contratam nossas inspeções mecânicas e laudos.</p>
                </div>

                {/* Horizontal Scrolling or Flexbox Grid of Client Logos */}
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 items-center justify-items-center">
                  {siteSettings?.partnerLogos && siteSettings.partnerLogos.length > 0 ? (
                    siteSettings.partnerLogos.map((client, idx) => (
                      <div
                        key={client.id || idx}
                        onClick={() => setZoomedLogo({ url: client.url, name: client.name, isSvg: false })}
                        className="w-full bg-white border border-slate-200/85 rounded-xl p-4 flex flex-col items-center justify-center text-center shadow-sm hover:shadow-md hover:border-emerald-500/30 transition-all duration-300 group h-28 cursor-pointer relative overflow-hidden"
                        id={`client-logo-custom-${idx}`}
                        title="Clique para ampliar"
                      >
                        {/* Hover Overlay for Zoom */}
                        <div className="absolute inset-0 bg-emerald-950/5 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity duration-300">
                          <div className="bg-white/95 p-1.5 rounded-full shadow-md text-emerald-700 transform scale-90 group-hover:scale-100 transition-transform duration-300">
                        ...
                        <div className="w-full flex items-center justify-center h-16">
                          <img
                            ...
                            referrerPolicy="no-referrer"
                            onError={(e) => {
                              (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=200&auto=format&fit=crop&q=60";
                            }}
                          />
                        </div>
                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1.5 block truncate max-w-full group-hover:text-emerald-700 transition-colors duration-300">{client.name}</span>
                      </div>
                    ))
                  ) : (
                    [
                      {
                        name: "VIDEPLAST",
                        logo: (
                          <svg viewBox="0 0 160 40" className="w-full h-11 max-h-11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <defs>
                              <linearGradient id="videplastRed" x1="0%" y1="0%" x2="100%" y2="100%">
                                <stop offset="0%" stopColor="#E21A22" />
                                <stop offset="100%" stopColor="#A81016" />
                              </linearGradient>
                              <linearGradient id="videplastYellow" x1="0%" y1="0%" x2="100%" y2="100%">
                                <stop offset="0%" stopColor="#FFE000" />
                                <stop offset="100%" stopColor="#FFA100" />
                              </linearGradient>
                            </defs>
                            <g transform="translate(4, 3)">
                              {/* Yellow ribbon backing */}
                              <path d="M12.5,4 C10.5,4.5 9,10 11,18 L15,31 L18.5,31 L13,10 C12.5,8 13.5,4 12.5,4 Z" fill="url(#videplastYellow)" />
                              {/* Red ribbon main fold */}
                              <path d="M5,4 C8,4 10,8 12.5,18 L17,31 L22,31 L12.5,4 Z" fill="url(#videplastRed)" />
                              {/* Red ribbon right swoosh fold */}
                              <path d="M22,31 C26.5,31 29,23.5 32,15 C34.5,9 38.5,8.5 41,9 C37,9.5 34.5,15.5 32,24 C29.5,31 25.5,31 22,31 Z" fill="url(#videplastRed)" />
                              <text x="46" y="27" fill="#3D454A" fontFamily="system-ui, -apple-system, sans-serif" fontWeight="900" fontSize="18" letterSpacing="0.5">VIDEPLAST</text>
                            </g>
                          </svg>
                        )
                      },
                        ...
                        logo: (
                          <svg viewBox="0 0 160 40" className="w-full h-11 max-h-11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g transform="translate(6, 2)">
                              <text x="0" y="27" fill="#007BC3" fontFamily="system-ui, -apple-system, sans-serif" fontWeight="800" fontSize="21" letterSpacing="-0.5">Prevema</text>
                              {/* Custom stylized X suffix */}
                              <path d="M91,12 L100,30" stroke="#007BC3" strokeWidth="4.5" strokeLinecap="round" />
                              <path d="M97,12 L105,21 L97,30" stroke="#F39200" strokeWidth="4.2" strokeLinecap="round" strokeLinejoin="round" />
                            </g>
                          </svg>
                        )
                      },
                      {
                        name: "GALA EMBALAGENS",
                        logo: (
                          <svg viewBox="0 0 160 40" className="w-full h-11 max-h-11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g transform="translate(8, 0)">
                              {/* Red arched swoosh */}
                              <path d="M 12 25 C 28 17.5, 58 17.5, 74 24 C 54 19.5, 26 19.5, 12 25 Z" fill="#E31E24" />
                              <text x="5" y="24" fill="#000000" fontFamily="system-ui, -apple-system, sans-serif" fontWeight="900" fontStyle="italic" fontSize="23" letterSpacing="1">GALA</text>
                              <text x="6" y="34" fill="#000000" fontFamily="system-ui, -apple-system, sans-serif" fontWeight="800" fontSize="7.5" letterSpacing="2.8">EMBALAGENS</text>
                            </g>
                          </svg>
                        )
                      },
                        ...
                        logo: (
                          <svg viewBox="0 0 160 40" className="w-full h-11 max-h-11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g transform="translate(5, 2)">
                              <text x="0" y="21" fill="#142B44" fontFamily="system-ui, -apple-system, sans-serif" fontWeight="800" fontSize="18" letterSpacing="-0.2">MaxiPlast</text>
                              <text x="1" y="32" fill="#64748B" fontFamily="system-ui, -apple-system, sans-serif" fontWeight="600" fontSize="8.5" letterSpacing="1.2">embalagens</text>
                              {/* Color wheel on the right */}
                              <g transform="translate(115, 20)">
                                <circle cx="0" cy="0" r="3" fill="#142B44" />
                                {/* 8 rounded rectangular petals rotating around center */}
                                <rect x="-2" y="-12" width="4" height="6.5" rx="1.5" fill="#00A3E0" />
                                <rect x="-2" y="-12" width="4" height="6.5" rx="1.5" fill="#4BBE2E" transform="rotate(45)" />
                                <rect x="-2" y="-12" width="4" height="6.5" rx="1.5" fill="#FFC72C" transform="rotate(90)" />
                                <rect x="-2" y="-12" width="4" height="6.5" rx="1.5" fill="#FF8F1C" transform="rotate(135)" />
                                <rect x="-2" y="-12" width="4" height="6.5" rx="1.5" fill="#E4002B" transform="rotate(180)" />
                                <rect x="-2" y="-12" width="4" height="6.5" rx="1.5" fill="#D6008D" transform="rotate(225)" />
                                <rect x="-2" y="-12" width="4" height="6.5" rx="1.5" fill="#7A3793" transform="rotate(270)" />
                                <rect x="-2" y="-12" width="4" height="6.5" rx="1.5" fill="#005A9C" transform="rotate(315)" />
                              </g>
                            </g>
                          </svg>
                        )
                      },
                      {
                        ...
                        logo: (
                          <svg viewBox="0 0 160 40" className="w-full h-11 max-h-11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g transform="translate(6, 2)">
                              {/* Divided diamond logo symbol */}
                              <g transform="translate(4, 3)">
                                {/* Left solid triangle */}
                                <path d="M 14,0 L 0,16 L 14,32 Z" fill="#004F9F" />
                                {/* Right diagonal bars */}
                                <path d="M 16,0 L 30,16 L 26,20 L 16,8 Z" fill="#0084D1" />
                                <path d="M 16,11 L 23,19 L 19,23 L 16,19 Z" fill="#0084D1" />
                                <path d="M 16,23 L 18,25 L 16,27 Z" fill="#0084D1" />
                              </g>
                              <text x="44" y="27" fill="#121C2B" fontFamily="system-ui, -apple-system, sans-serif" fontWeight="800" fontSize="18" letterSpacing="1.2">SULFIBRA</text>
                            </g>
                          </svg>
                        )
                      },
                        ...
                        logo: (
                          <svg viewBox="0 0 160 40" className="w-full h-11 max-h-11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g transform="translate(10, 0)">
                              {/* Double arched green ribbons above */}
                              <g transform="translate(0, 5)">
                                <path d="M 12,12 C 22,5 34,5 44,9 C 34,9.5 24,11 12,12 Z" fill="#4CA135" />
                                <path d="M 5,19 C 18,10 32,10 42,14 C 30,15 20,18 5,19 Z" fill="#4CA135" />
                              </g>
                              <text x="44" y="28" fill="#4CA135" fontFamily="system-ui, -apple-system, sans-serif" fontWeight="800" fontSize="21" letterSpacing="-0.5">master</text>
                            </g>
                          </svg>
                        )
                      }
                      ...
                      <div
                        key={idx}
                        onClick={() => setZoomedLogo({ name: client.name, isSvg: true, svg: client.logo })}
                        className="w-full bg-white border border-slate-200/85 rounded-xl p-4 flex flex-col items-center justify-center text-center shadow-sm hover:shadow-md hover:border-emerald-500/30 transition-all duration-300 group h-28 cursor-pointer relative overflow-hidden"
                        id={`client-logo-${idx}`}
                        title="Clique para ampliar"
                      >
                        ...
                        <div className="w-full flex items-center justify-center h-16 group-hover:scale-105 transition-transform duration-300">
                          {client.logo}
                        </div>
                      </div>
                    ))
